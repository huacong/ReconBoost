...
class MeshNet(nn.Module):
    ...
    def forward(self, x, global_ft=False, middle_fts = None):
        ...
        gt_fea = self.classifier[:1](fea)
        #### feature fusion
        if middle_fts is not None:
            # gt_fea = self.middle_fusion(torch.cat([gt_fea,middle_fts],dim=1))
            gt_fea = F.relu(self.middle_bn(self.middle_fusion(torch.cat([gt_fea,middle_fts],dim=1))))
        #### 
        fea = self.classifier[1:-1](gt_fea)
        cls = self.classifier[-1:](fea)

        if global_ft:
            return cls, gt_fea / torch.norm(gt_fea)
        else:
            return cls
